...
export async function writeWorkflowArtifacts(
  ...
  await writeJsonFile(
    ...
    sanitizeForPersistence(summary),
  );
  await writeJsonFile(
  ...
  await writeJsonFile(
    path.join(outputDir, 'defects.json'),
    sanitizeForPersistence(summary.defects),
  );
  await writeJsonFile(
...
export function buildDefectsFromExecution(
  ...
  return requirements
    .slice(0, Math.min(requirements.length, execution.failed))
    .map((requirement) => ({
