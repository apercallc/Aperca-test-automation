...
test.describe('Product discovery', () => {
  test('products page supports search and product detail navigation', async ({
    page,
  }) => {
    ...
    await page
      ...
      .fill('FastCarb');
    await expect(
      page.getByRole('heading', { name: 'FastCarb', exact: true }),
    ).toBeVisible();

    await page
      .getByRole('link', { name: /FastCarb/ })
      .first()
      .click();
    ...
    ).toHaveAttribute('href', /fastcarb\.apercallc\.com/);
    await expect(
      page.getByRole('heading', { name: 'About FastCarb' }),
    ).toBeVisible();
  });
});
